#!/usr/bin/env bash
# 每日备份薄入口：实际逻辑全部在 deploy/backup.py（唯一格式 skill-eval-backup/v1）。
#
...
#
# 服务器上的安装位置：/opt/skill-eval/backup.sh 与 /opt/skill-eval/backup.py
# 首次使用前：
#   1. 在 /opt/skill-eval/.env 填好 OSS_BUCKET、OSS_PREFIX、OSS_ENDPOINT
...
COMPOSE_DIR="${COMPOSE_DIR:-/opt/skill-eval}"
