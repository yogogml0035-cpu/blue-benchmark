...
import { Plus } from "@/src/components/ui/Glyph";
...
import styles from "./WorkspaceShelf.module.css";
...
export function WorkspaceShelf() {
      ...
      <>
        ...
        <main className="page page-mid">
          <StatePanel
            ...
            description="卷宗架只属于登录用户。会话失效或尚未登录时，这里不会渲染任何私有内容。"
            title="需要登录才能打开卷宗架"
            tone="locked"
          />
        </main>
  ...
  // 架子是空的时候，创建表单就是这一屏的焦点；有卷宗时它退回成一个可展开的动作。
      ...
      <main className="page page-mid stack-lg">
        <div className="row-between">
          <div className="stack-sm">
            <h1 className="doc-title">卷宗架</h1>
            <p className="secondary">
              每一份卷宗是一个私有场景。案例、草案和候选用例都继承卷宗的归属，其他账号读不到。
            </p>
          </div>
          {!empty && !loading && (
            <Button
            ...
            >
              {composerOpen ? "收起" : <><Plus />新建卷宗</>}
            </Button>
          )}
        </div>

        {created && (
          <Note tone="cleared" title={`已建立「${created.name}」`}>
            <span className="row" style={{ gap: "var(--s-3)" }}>
              <span className="mono">{created.id}</span>
              <Link href={`/workspaces/${created.id}/cases/new`}>向这份卷宗收件 →</Link>
            </span>
          </Note>
        )}
        ...
        {composerVisible && (
          <section className="sheet enter">
            <div className="sheet-head">
              <h2 className="doc-title-sm">{empty ? "先建立第一份卷宗" : "新建卷宗"}</h2>
              {empty && (
            ...
            <form className="sheet-pad stack" onSubmit={submit}>
              ...
              {createFault && (
                <Note code={createFault.code} title="卷宗没有建立" tone="fail">
                  {createFault.message}
                </Note>
              )}
              <div className="row">
        ...
        <section className="stack">
          <div className="spread">
            <span className="section-label">我的卷宗</span>
            <span className="mono faint">{loading ? "…" : `${items.length} 份`}</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="inset stack-sm" style={{ padding: "var(--s-6)", textAlign: "center" }}>
              <p className="secondary">架子还是空的。</p>
              <p className="mono faint">GET /api/workspaces → items: []</p>
            </div>
          ) : (
            <div className={styles.shelf}>
              {items.map((workspace) => (
                <article className={`${styles.folder} enter`} key={workspace.id}>
                  <div className={styles.folderHead}>
                    <h3 className={styles.folderName}>{workspace.name}</h3>
                    {workspace.description ? (
                      <p className={styles.folderNote}>{workspace.description}</p>
                    ) : (
                      <p className={styles.folderNote} style={{ color: "var(--ink-faint)" }}>
                        无说明
                      </p>
                    )}
                  </div>
