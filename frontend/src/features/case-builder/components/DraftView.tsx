...
import { FieldRow, ItemText, RefsDetail, SourceItem } from "@/src/features/case-builder/components/SourceGroup";
...
/** 维度类型用纯文字后缀表达（事实准确性 · 硬门槛），不做徽标。 */
function KindSuffix({ kind }: { kind: DimensionKind }) {
  return <span className={styles.kindSuffix}>· {KIND_LABEL[kind]}</span>;
}
...
/** 聚焦阅读的节：DraftView 一次只渲染当前节；"all" 用于已定稿的只读快照。 */
  ...
  return (
    <div className={styles.rows}>
      {values.map((value, index) => (
        <div className={styles.rowPlain} key={index}>
          <input
      ...
      <div>
        <Button disabled={busy} onClick={() => onChange([...values, ""])} size="sm" variant="quiet">
          添加{addLabel}
        </Button>
      </div>
    </div>
...
function StringList({ values, empty }: { values: string[]; empty: string }) {
  ...
  return (
    <ul className={styles.rows}>
      {values.map((value, index) => (
        <li className={styles.fieldText} key={index}>
          {value}
        </li>
      ))}
    </ul>
...
function SubGroup({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className={styles.subGroup}>
      <span className={styles.subGroupLabel}>{title}</span>
      {children}
    </div>
  );
}
 ...
 * 题稿：读和改共用同一份结构与字段顺序，
 * 避免定稿卡和只读快照长成两个不同的东西。
 * section="all" 渲染全部（已定稿快照）；聚焦阅读时一次一节。
 */
export function DraftView({
  ...
  const show = (key: DraftSection) => section === "all" || section === key;

  return (
    <div className={section === "all" ? styles.groupsAll : styles.groups}>
      {show("summary") && (
        <FieldRow label={section === "all" ? "场景摘要" : ""} refs={draft.scenario.evidence_refs ?? []}>
          {editable ? (
            <AutoTextarea
              aria-label="场景摘要"
              className="control"
      ...
      {show("outcome") && (
        <FieldRow label={section === "all" ? "参考结果" : ""} refs={draft.reference_outcome?.evidence_refs ?? []}>
          {editable ? (
        ...
        <div className={styles.rows}>
          {draft.dimensions.map((dimension, index) => (
            <SourceItem
              ...
              badge={editable ? addedBadge(dimension.id) : undefined}
              key={dimension.id}
              ...
              {editable ? (
                ...
                <div className="stack-sm">
                  <span className={styles.fieldText} style={{ fontWeight: 550 }}>
                    ...
                    <KindSuffix kind={dimension.kind} />
                  </span>
              ...
              >
                {editable ? (
                  <div className="stack-sm">
                    ...
                    />
                    {unknown.blocking && (
                      <p className="field-error">阻塞缺口必须先解决，才能定稿。</p>
                    )}
                  </div>
                ) : (
