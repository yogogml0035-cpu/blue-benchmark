...
class EvaluationSetVersionRow(Base):
    ...
    draft_id: Mapped[str | None] = mapped_column(ForeignKey("working_set_drafts.id"), nullable=True, index=True)
    contract_revision_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
...
class BenchmarkQuestionDraftRow(Base):
    ...
    bad_samples_json: Mapped[list] = mapped_column(JSON, default=list)
    ...
    lifecycle_receipts_json: Mapped[dict] = mapped_column(JSON, default=dict)
    lifecycle_pending_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
