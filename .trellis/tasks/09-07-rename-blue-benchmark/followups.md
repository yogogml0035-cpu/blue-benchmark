...
3. [P2] deploy/backup.py:458-460 `_discover_schema_version` 探测 `compose_dir.parent/"backend"/"alembic"/"versions"`,实际迁移目录是 `backend/migrations/versions`,且服务器 /opt 布局根本没有 repo 检出 → manifest `business_schema_version` 恒 None,restore.md:79 要求核对的"业务 schema 版本"永远显示"未知"。需要设计决策(从库内 alembic_version 读取 vs 删除文档核对项)。
