...
真实闭环仍按 `README.md` 人工验收：注册、建场景、上传、题目边界、组合确认动作、自动发布、停用/恢复/删除、历史下载、刷新恢复、跨账号 403 和错误恢复。只有实际执行后才能声称浏览器路径跑通。
...
- [ ] 已识别的 `preview` 状态不调用任何真实 `/api/` 资源；预演 E2E 在 API 未启动时监听请求、console 和 pageerror，确认 fixture 与真实 Session 完全隔离。
- [ ] 导航型组合命令在服务端成功到路由离开前保持 busy 和稳定 command，不因短暂的页面切换窗口重复提交并制造 409。
