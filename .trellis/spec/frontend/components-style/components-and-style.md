...
- 题目材料浏览态按模块包裹：模块 `width: fit-content; max-width: 100%`，内容区 `max-height: 280px; overflow-y: auto`；短内容自然收缩，长内容在当前材料栏宽度内换行。多条材料共用一个可聚焦、有名称的滚动区，条目不再单独限高或滚动；标题与编辑入口留在滚动区外。编辑态展开到材料栏宽度，保留模块内切换不保存、移出模块自动保存的行为。浏览器回归覆盖 `1440x900` / `1280x720`、连续长字符、多条材料与键盘滚动。
