...
适用于 `backend/` 的 FastAPI 内存 Stub。写代码前先确认任务触及哪一类边界，再读取对应规范。
...
| [Stub 状态、数据与合同](./stub-state-and-contracts.md) | 修改内存数据、认证归属、上传、Case 状态机或确认流程 |
| [错误与 API 响应](./errors-and-api-responses.md) | 新增错误分支、状态码、异常处理或前后端错误合同 |
| [后端质量与测试](./quality-and-tests.md) | 编写或审查后端代码、补充 API 测试、执行质量门 |

## 开发前检查

- [ ] 已从源码和 `README.md` 判断当前实现，没有把数据库、LangGraph、真实 AI 等未来文档当成现状。
