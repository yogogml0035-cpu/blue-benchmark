...
M0 业务码包括 `COMMAND_ID_REUSED`、`COMMAND_IN_PROGRESS`、`CASE_ALREADY_EXISTS`、`BATCH_CASE_INVALID`、`PRIVATE_CONTENT_REJECTED`、`CRITERION_TOO_VAGUE`、`STALE_REVISION`、`RUBRIC_GENERATING`、`GENERATION_FAILED`、`CRITERIA_MISSING`、`CRITERIA_NOT_CONFIRMED`、`ALREADY_PUBLISHED`、`REVIEW_REOPEN_NOT_AVAILABLE`、`PUBLISHED_REOPEN_REQUIRED`、`DELETE_CONFIRMATION_MISMATCH`、`SCENE_NOT_EMPTY`、`RETRY_NOT_AVAILABLE`、`NO_MATERIAL_CHANGE`、`ADMIN_EXISTS`、`ADMIN_MISSING`、`PASSWORD_INVALID`、`SCENE_NAME_EXISTS`、`CREDENTIAL_ALREADY_REVOKED`；它们遵守 `409`（状态/版本/幂等冲突）或 `422`（输入/材料/标准/确认无效）的语义。
