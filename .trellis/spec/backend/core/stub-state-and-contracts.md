...
当前后端是统一的评测题库：业务数据库（SQLAlchemy + Alembic）保存题目六类材料、两字段评分维度、场景与凭证、批量收题回执和评分生成任务；恰好一个生产 Worker 负责评分维度生成。前端（`frontend/`）只做呈现，不持有业务事实；后端没有文件上传解析。`ai_runtime/deep_runtime.py` 提供 Deep Agents 受限运行与 PostgreSQL 检查点的底层原语（加密序列化、同连接线程单写者锁、恢复分类和无模型清理），但未接入生产评分链路，也没有业务 thread/公开消息表。规范以源码、测试和迁移为准；规划文档中尚未落地的能力不能当作当前事实。
...
| `deep_runtime` 受限 Agent/事件归一/PG 检查点会话/线程锁/清理原语（独占测试库与真实 Provider 探针验证） | 业务 thread 映射、公开消息持久化、SSE 与生产 Worker 接入 |
