...
3. 评分维度生成（成功、失败、重试、孤立标签拒绝、建议分锚点/依据完整性、同命令复活不污染队列）；
4. 状态机与发布门禁、材料自动保存（`PATCH /materials` 不推进 revision、不动维度）、维度字段自动保存（`PATCH /criteria/{id}`）、无条件 `regenerate` 作废旧维度、`title` 改名不触发重新生成；
5. 完整评分项合同（维度+整数通过分+锚点+双依据+分类主张+可核查引用）与逐项及格语义；老师编辑不受锚点白名单约束、依据不被静默改写；
