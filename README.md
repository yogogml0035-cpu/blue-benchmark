...
这是 M0 统一题库后端：业务老师在本地 Agent 里完成真实任务后，调用仓库内上传 Skill，从当前可见上下文整理评测题并经确认后批量上传；后端保存题目的六类材料，并自动为每道题生成两字段评分维度（`criterion + pass_score`，固定 10 分、逐项及格），供后续新前端与评测运行使用。
...
- **统一题库**：所有待处理与已发布题目在同一个题库中，按状态、筛选和下一步动作区分；可建多个**场景文件夹**只作归类。
- **一道题的六类材料**：题目、参考样例、Bad case（绑定老师反馈）、标准答案、记忆材料，以及仅供检索的用例标题 `title`。
