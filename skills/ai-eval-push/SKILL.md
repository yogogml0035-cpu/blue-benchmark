---
name: ai-eval-push
description: 从当前 Agent 上下文整理评测题，并上传到「蓝标汽车事业 BenchMark 平台」题库。只要用户明确要求把刚才一起完成的工作保存、提炼、收集或上传为评测题 / 测试用例 / 基准用例——即使没有说"上传"这个词——都应使用本 Skill。它会识别候选题目、整理六类材料、让老师对整批预览确认，然后用场景凭证上传。在用户确认之前绝不上传。
---
